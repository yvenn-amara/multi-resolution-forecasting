...
rolling_test = pd.read_csv('Data/low-resolution_test.csv')
...
selection = features + ["tod24","dow", "load", 'dateDay', 'ymd', 'ym']
...
model.save('./weights/low-resolution_FC_rolling')
...
for i in range(len(ym_unique)-12):
    print("================ Iteration",i+1, "================")
    model = load_model('./weights/low-resolution_FC_rolling')
...
for i in range(48):
    path = '2. Pred Signals/2. NNs/1. FC/fitted/low-resolution_FC_rolling_' + str(i) + '.csv'    
    np.savetxt(path, np.vstack(fitted_signal[i]), delimiter=',')

np.savetxt('2. Pred Signals/2. NNs/1. FC/low-resolution_FC_rolling_pred.csv', pred_signal, delimiter=',')
